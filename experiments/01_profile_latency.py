...
Schema: task, model, variant, device, batch_size, lat_p50_ms, lat_p95_ms,
        lat_mean_ms, lat_std_ms, throughput_items_per_sec, num_measurements
"""
...
def load_sst2_samples(n=100):
    ...
    LOGGER.info(
        ...
        f"(lengths: {min(len(t.split()) for t in texts)}"
        f"-{max(len(t.split()) for t in texts)} words)"
    )
    return texts
...
def profile_text_model(model_name, variant, device, batch_size, texts):
    ...
    LOGGER.info(
        f"  p50={measurements['lat_p50_ms']:.2f}ms, "
        f"p95={measurements['lat_p95_ms']:.2f}ms, "
        f"throughput={measurements['throughput_items_per_sec']:.1f} items/s"
    )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick test mode (fewer iterations and configs)"
    )
            ...
            for device in DEVICES:
                ...
                for batch_size in BATCH_SIZES:
                    try:
                        ...
                        LOGGER.error(
                            f"Failed to profile {model_name} "
                            f"({variant}, {device}, batch={batch_size}): {e}"
                        )
    ...
    for model_name in IMAGE_MODELS:
        for variant in VARIANTS:
            for device in DEVICES:
                ...
                for batch_size in BATCH_SIZES:
                    try:
                        result = profile_image_model(
                            model_name, variant, device, batch_size, images
                        )
                        results.append(result)
                    except Exception as e:
                        LOGGER.error(
    ...
    # Summary statistics
    text_rows = df[df["task"] == "text"]
    vision_rows = df[df["task"] == "vision"]
    LOGGER.info("\nSummary:")
    LOGGER.info(f"  Text models: {text_rows.shape[0]} configurations")
    LOGGER.info(f"  Image models: {vision_rows.shape[0]} configurations")
    if not text_rows.empty:
        text_latency = text_rows["lat_p50_ms"]
        LOGGER.info(f"  Latency range (text): {text_latency.min():.2f}-{text_latency.max():.2f} ms")
    if not vision_rows.empty:
        vision_latency = vision_rows["lat_p50_ms"]
        LOGGER.info(
            f"  Latency range (vision): {vision_latency.min():.2f}-{vision_latency.max():.2f} ms"
        )
