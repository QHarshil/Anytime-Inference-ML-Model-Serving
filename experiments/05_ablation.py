...
def ablation_model_size(latency_df, accuracy_df, task):
    ...
    for small_model, large_model in pairs:
        ...
        if len(small_lat) > 0 and len(large_lat) > 0:
            ...
            acc_change = (
                ((large_acc_val - small_acc_val) / small_acc_val) * 100
                if small_acc_val > 0
                else 0.0
            )
        ...
        for quant_variant in ["fp16", "int8"]:
            if quant_variant in variants:
                ...
                if len(fp32_lat) > 0 and len(quant_lat) > 0:
                    ...
                    acc_change = (
                        ((quant_acc_val - fp32_acc_val) / fp32_acc_val) * 100
                        if fp32_acc_val > 0
                        else 0.0
                    )
    ...
    if len(batch_sizes) >= 2:
        ...
        for comparison_batch in batch_sizes[1:]:
            ...
            if len(baseline_lat) > 0 and len(comparison_lat) > 0:
                ...
                throughput_change = (
                    (throughput_comparison - throughput_baseline) / throughput_baseline
                ) * 100
...
def ablation_device(latency_df, accuracy_df, task):
    ...
    if "cpu" in devices and "cuda" in devices:
        ...
        if len(cpu_lat) > 0 and len(gpu_lat) > 0:
            ...
            acc_change = (
                ((gpu_acc_val - cpu_acc_val) / cpu_acc_val) * 100 if cpu_acc_val > 0 else 0.0
            )
...
def ablation_cascade(planner_results_df, baseline_results_df, task):
    ...
    single = baseline_results_df[
        (baseline_results_df["task"] == task) & (baseline_results_df["method"] == "StaticLarge")
    ]
...
def main():
    ...
    planner_results_df = (
        pd.read_csv(results_dir / "planner_results.csv")
        if (results_dir / "planner_results.csv").exists()
        else pd.DataFrame()
    )
    baseline_results_df = (
    ...
    for task in ["text", "vision"]:
        ...
        for r in results:
            LOGGER.info(
                f"    {r['factor']}: latency {r['latency_change_pct']:+.1f}%, "
                f"accuracy {r['accuracy_change_pct']:+.1f}%"
            )
        ...
        for r in results:
            LOGGER.info(
                f"    {r['factor']}: latency {r['latency_change_pct']:+.1f}%, "
                f"throughput {r['throughput_change_pct']:+.1f}%"
            )
