...
Schema: method, task, deadline_ms, seed, deadline_hit_rate, accuracy,
        accuracy_std, lat_p95_ms, lat_std_ms, num_runs, num_samples
"""
...
from anytime_serving.evaluation.real_inference import RealInferenceEvaluator
...
def select_throughput_autotuner(
    latency_df: pd.DataFrame, accuracy_df: pd.DataFrame, task: str, deadline_ms: float
) -> dict:
    ...
    feasible["throughput"] = feasible.get(
        "items_per_sec", feasible.get("throughput_items_per_sec", 1000.0 / feasible["lat_p50_ms"])
    )
...
def select_infaas_style(
    ...
    merged = task_lat.merge(
        task_acc[["model", "variant", "accuracy"]], on=["model", "variant"], how="inner"
    )

    # Filter by constraints
    feasible = merged[
        (merged["lat_p95_ms"] < deadline_ms) & (merged["accuracy"] >= accuracy_target)
    ]
...
def evaluate_baseline(
    ...
    model, tokenizer_or_transform = model_zoo.load_model(
        task=task, model_name=config["model"], variant=config["variant"], device=config["device"]
    )
    ...
    return {
        ...
        "config_selected": (
            f"{config['model']}_{config['variant']}_{config['device']}_b{config['batch_size']}"
        ),
...
def main():
    ...
    for task in ["text", "vision"]:
        ...
        for deadline_ms in deadlines[task]:
            ...
            config_throughput = select_throughput_autotuner(
                latency_df, accuracy_df, task, deadline_ms
            )
    ...
    # Summary
    summary = (
        results_df.groupby(["method", "task"])
        .agg({"deadline_hit_rate": ["mean", "std"], "accuracy": ["mean", "std"]})
        .round(3)
    )
    LOGGER.info(f"\nSummary:\n{summary}")
