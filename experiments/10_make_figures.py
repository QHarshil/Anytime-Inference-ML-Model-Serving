...
def plot_pareto_frontier(pareto_df, baseline_df, planner_df, output_dir):
    ...
    for idx, task in enumerate(["text", "vision"]):
        ...
        for method in task_baseline["method"].unique():
            ...
            ax.scatter(
                method_data["lat_p95_ms"], method_data["accuracy"], label=method, alpha=0.6, s=50
            )
...
def plot_baseline_comparison(baseline_df, planner_df, output_dir):
    ...
    baseline_agg = (
        baseline_df.groupby(["method", "task"])
        .agg({"deadline_hit_rate": ["mean", "std"], "accuracy": ["mean", "std"]})
        .reset_index()
    )

    planner_agg = (
        planner_df.groupby("task")
        .agg({"deadline_hit_rate": ["mean", "std"], "accuracy": ["mean", "std"]})
        .reset_index()
    )
    ...
    # Combine
    combined = pd.concat([baseline_agg, planner_agg])
    ...
    for idx, task in enumerate(["text", "vision"]):
        ...
        ax.bar(
            x,
            task_data[("deadline_hit_rate", "mean")],
            yerr=task_data[("deadline_hit_rate", "std")],
            capsize=5,
        )
        ax.set_xticks(x)
...
def plot_ablation_studies(ablation_df, output_dir):
    ...
    for idx, ablation_type in enumerate(ablation_types):
        ...
        ax.bar(
            x - width / 2, type_data["latency_change_pct"], width, label="Latency Change", alpha=0.8
        )
        ax.bar(
            x + width / 2,
            type_data["accuracy_change_pct"],
            width,
...
def main():
    ...
    pareto_df = (
        pd.read_csv(results_dir / "pareto_analysis.csv")
        if (results_dir / "pareto_analysis.csv").exists()
        else pd.DataFrame()
    )
    ablation_df = (
        pd.read_csv(results_dir / "ablation_results.csv")
        if (results_dir / "ablation_results.csv").exists()
        else pd.DataFrame()
    )
    workload_df = (
        pd.read_csv(results_dir / "workload_sensitivity.csv")
        if (results_dir / "workload_sensitivity.csv").exists()
        else pd.DataFrame()
    )
    stats_df = (
        pd.read_csv(results_dir / "statistical_tests.csv")
