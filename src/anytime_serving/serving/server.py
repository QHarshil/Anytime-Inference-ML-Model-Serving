...
class AdaptiveServer:
    ...
    def __init__(
        ...
        *,
        ...
        if selector.servers != runtime_pool.size:
            raise ValueError(
                f"selector models {selector.servers} server(s) but the runtime pool has "
                f"{runtime_pool.size}. The admission controller would misjudge queue "
                f"capacity by a factor of {runtime_pool.size / selector.servers:.3g}."
            )
        self._pool = runtime_pool
        self._selector = selector
        self._load_monitor = load_monitor
        self._executor = ThreadPoolExecutor(
            max_workers=max_in_flight if max_in_flight else runtime_pool.size
        )
        ...
        # Requests admitted but not yet finished. The dispatch queue is unbounded,
        # so this is what actually determines how long a new arrival waits.
        self._in_flight = 0
    ...
    def submit(
        ...
        decision = self._selector.select(
            deadline_ms, arrival_rate, load_percent, queue_depth=queue_depth
        )
        ...
        # Count the request in flight before it is queued, so a burst arriving
        # faster than the pool drains is visible to the next admission decision.
        with self._in_flight_lock:
            self._in_flight += 1
        try:
            ...
            # The executor was shut down; do not leak the in-flight count.
            with self._in_flight_lock:
                self._in_flight -= 1
            raise
    ...
    def _run(
        ...
        *,
        ...
        try:
            ...
            # Released on every path, so a runtime failure cannot strand the
            # counter and permanently depress admission.
            with self._in_flight_lock:
                self._in_flight -= 1
...
def poisson_arrivals(
    duration_s: float, rate_rps: float, *, rng: np.random.Generator | None = None
) -> list[float]:
