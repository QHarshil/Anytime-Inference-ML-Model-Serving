...
class RealInferenceEvaluator:
    ...
    def _get_cache_key(self, config: dict, task: str, seed: int) -> str:
        ...
        return (
            f"{task}_{config['model']}_{config['variant']}"
            f"_{config['device']}_{config['batch_size']}_seed{seed}"
        )
    ...
    def evaluate_config(
        ...
        result = InferenceResult(
            ...
            latencies_ms=[float(x) for x in all_latencies],
...
class CascadeInferenceEvaluator:
    ...
    def evaluate_cascade(
        ...
        for run_idx in range(num_runs):
            ...
            stage2_positive = [lat for lat in run_stage2_latencies if lat > 0]
